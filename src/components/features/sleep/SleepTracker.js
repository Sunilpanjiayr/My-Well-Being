...
function SleepTracker() {
  ...
  const [sleepGoal, setSleepGoal] = useState(() => parseFloat(localStorage.getItem('sleepGoal')) || 8);
  ...
  // New Features
  const [sleepPhases, setSleepPhases] = useState({ deep: 0, light: 0, rem: 0, awake: 0 });
  ...
  const sleepTips = [
    {
      ...
      details: {
        recommended: 'Set a fixed bedtime and wake-up time, even on weekends, aiming for ±30 minutes consistency.',
    ...
    },
    {
      ...
      details: {
        recommended: 'Dark room with blackout curtains, temperature between 60-67°F (15-19°C), minimal noise.',
        avoid: 'Bright screens, uncomfortable temperature, disruptive sounds or lights.',
    ...
    },
    {
      ...
      details: {
        recommended: 'Turn off all screens 1-2 hours before bed, use night mode or blue light filters if necessary.',
        avoid: 'Social media, news, work emails, or stimulating content before sleep.',
    ...
    },
    {
      ...
      description: 'Optimize your evening meals and beverages for better sleep.',
      details: {
        recommended: 'Light, easy-to-digest meals 2-3 hours before bedtime, calming teas like chamomile.',
        avoid: 'Caffeine after noon, alcohol before bed, heavy or spicy meals, excessive liquids close to bedtime.',
    ...
    },
    {
      ...
      details: {
        recommended: 'Regular exercise, ideally earlier in the day, gentle stretching or yoga in the evening.',
        avoid: 'Intense workouts within 2-3 hours of bedtime, which can raise core temperature and alertness.',
    ...
    },
    {
      ...
      details: {
        recommended: 'Deep breathing exercises, progressive muscle relaxation, journaling, gratitude practice.',
        avoid: 'Work tasks, stressful conversations, planning, or problem-solving right before bed.',
    ...
    },
    {
      ...
      details: {
        recommended: 'Morning sunlight exposure, dimming lights in the evening, complete darkness for sleep.',
        avoid: 'Bright lights at night, irregular sleep-wake times, daytime darkness.',
    ...
    },
    {
      ...
      details: {
        recommended: 'Short power naps (20-30 minutes) early afternoon, not after 3PM.',
        avoid: 'Long naps, late afternoon or evening napping, using naps to compensate for poor night sleep.',
    ...
    },
    {
      ...
      details: {
        recommended: 'Calibrate devices properly, establish baseline measurements, focus on trends rather than daily fluctuations.',
        avoid: 'Obsessing over sleep data, using intrusive devices that disrupt sleep, comparing to others.',
    ...
    },
    {
      ...
      details: {
        recommended: 'Gradual recovery by adding 15-30 minutes extra sleep per night, consistent wake times.',
        avoid: 'Weekend oversleeping, irregular catch-up patterns, daytime hibernation.',
  ...
  const recommendedDevices = [
    {
      ...
      details: 'The Oura Ring provides comprehensive sleep stage analysis, heart rate variability, and temperature monitoring in a discreet form factor that doesn\'t disturb sleep.',
      features: ['Sleep Stages', 'HRV Tracking', 'Body Temperature', 'Readiness Score', 'Activity Tracking'],
      price: '$299.00',
    ...
    },
    {
      ...
      details: 'This non-wearable device fits under your mattress and tracks sleep cycles, heart rate, snoring, and even detects sleep apnea episodes with medical-grade accuracy.',
      features: ['Sleep Cycle Analysis', 'Heart Rate Monitoring', 'Snoring Detection', 'Sleep Apnea Detection', 'Smart Home Integration'],
      price: '$129.95',
    ...
    },
    {
      ...
      details: 'The Dreem 2 uses EEG sensors to monitor brain activity during sleep, providing the most accurate sleep staging available in consumer technology, plus sound stimulation to enhance deep sleep.',
      features: ['EEG Monitoring', 'Brain Activity Tracking', 'Deep Sleep Stimulation', 'Detailed Sleep Architecture', 'Personalized Coaching'],
      price: '$499.00',
    ...
    },
    {
      ...
      details: 'This advanced mattress cover provides dynamic temperature regulation for each side of the bed, tracks sleep phases, and integrates with your smart home for the optimal sleep environment.',
      features: ['Dual-Zone Temperature Control', 'Sleep Tracking', 'Smart Alarm', 'Heart Rate Monitoring', 'HRV Analysis'],
      price: '$1,695.00',
    ...
    },
    {
      ...
      details: 'Designed for those who don\'t get enough sleep, this headband detects when you\'re in deep sleep and delivers customized audio tones to enhance the quality of this restorative sleep phase.',
      features: ['Slow Wave Sleep Enhancement', 'Sleep Tracking', 'Smart Alarm', 'Insights & Coaching', 'Battery Life: 4+ Nights'],
      price: '$399.95',
    ...
    },
    {
      ...
      details: 'This bedside device uses echolocation technology to monitor your breathing and movement without any wearables, providing detailed sleep analysis and personalized advice.',
      features: ['Contactless Monitoring', 'Light & Temperature Analysis', 'Comprehensive Sleep Scores', 'Science-Based Advice', 'Room Environment Optimization'],
      price: '$149.99',
  ...
  useEffect(() => {
    ...
    setHealthConnections({
      ...
      heartRate: localStorage.getItem('healthConnectionHeartRate') === 'true' || false,
      stress: localStorage.getItem('healthConnectionStress') === 'true' || false
    });
  }, []);
  ...
  useEffect(() => {
    ...
    localStorage.setItem('healthConnectionHeartRate', healthConnections.heartRate);
  ...
  }, [sleepLogs, sleepGoal, reminderTime, reminderEnabled, darkMode, healthConnections]);

  // Calculate duration when bedtime/wakeup changes
  useEffect(() => {
    if (newSleepLog.bedtime && newSleepLog.wakeup) {
      const calculatedDuration = calculateDuration(newSleepLog.bedtime, newSleepLog.wakeup);
      setNewSleepLog(prev => ({
        ...prev,
  ...
  const calculateWeeklyProgress = () => {
    ...
    const daysMetGoal = thisWeekLogs.filter(log => parseFloat(log.duration) >= sleepGoal).length;
  ...
  const calculateSleepTrends = () => {
    ...
    // Calculate averages
    const last7Avg = last7Days.reduce((sum, log) => sum + parseFloat(log.duration), 0) / last7Days.length;
    const previous7Avg = previous7Days.length > 0 
      ? previous7Days.reduce((sum, log) => sum + parseFloat(log.duration), 0) / previous7Days.length 
      : 0;
  ...
  const calculateSleepPhases = () => {
    ...
    const totalDeep = logsWithPhases.reduce((sum, log) => sum + (log.sleepPhases.deep || 0), 0);
    const totalLight = logsWithPhases.reduce((sum, log) => sum + (log.sleepPhases.light || 0), 0);
    const totalRem = logsWithPhases.reduce((sum, log) => sum + (log.sleepPhases.rem || 0), 0);
    const totalAwake = logsWithPhases.reduce((sum, log) => sum + (log.sleepPhases.awake || 0), 0);
  ...
  const getSleepPatterns = useCallback(() => {
    ...
    const totalDuration = sleepLogs.reduce((sum, log) => sum + (parseFloat(log.duration) || 0), 0);
    ...
    const sleepScore = scoreFactors.length
      ? Math.round(scoreFactors.reduce((sum, s) => sum + s, 0) / scoreFactors.length)
      : 0;

    let streak = 0;
    const sortedLogs = [...sleepLogs].sort((a, b) => new Date(b.date) - new Date(a.date));
    ...
    const consistency = bedtimes.length
      ? 100 - Math.round(
          (bedtimes.reduce((sum, time) => sum + Math.abs(time - avgBedtime), 0) /
            bedtimes.length /
            60) *
            20
        )
